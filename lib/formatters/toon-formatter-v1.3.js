...
class ToonFormatterV13 {
    ...
    parseValue(lines, startIndex, expectedDepth) {
        ...
        if (trimmed.includes(':')) {
            ...
            if (valueStr) {
                // Check if the value looks like an inline array (e.g., "[3]: 1,2,3")
                if (valueStr.match(/^\[[#]?\d+([,\t|])?\]:/)) {
                    // Parse as inline array
                    const arrayResult = this.parseArray([trimmed], 0, expectedDepth);
                    ...
                    if (nextLine && nextLine.trim() && this.getIndentDepth(nextLine) === currentDepth && nextLine.trim().includes(':')) {
                        // Multiple properties - parse as object but with array value for this key
                        return this.parseObject(lines, startIndex, expectedDepth);
                    }

                    // Single property with array value
                    return {
                        value: { [this.unquoteKey(key)]: arrayResult.value },
    ...
    parseObject(lines, startIndex, depth) {
        ...
        while (index < lines.length) {
            ...
            if (valueStr) {
                // Check if value is an inline array (e.g., "[3]: 1,2,3")
                if (valueStr.match(/^\[[#]?\d+([,\t|])?\]:/)) {
                    const arrayResult = this.parseArray([trimmed], 0, depth);
