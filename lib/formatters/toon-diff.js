...
class ToonDiff {
    ...
    static applyPatch(obj, patch) {
        ...
        // Sort changes to handle array operations correctly
        // Array removals must be processed from highest index to lowest
        // to avoid index shifting issues
        const sortedChanges = [...patch.changes].sort((a, b) => {
            // If both are array operations, sort by index (descending for removals)
            if (a.type === 'item_removed' && b.type === 'item_removed') {
                const aIndex = this._getArrayIndex(a.path);
                const bIndex = this._getArrayIndex(b.path);
                return bIndex - aIndex; // Descending order
            }
            // Keep original order for other operations
            return 0;
        });
    ...
    /**
     * Helper: Extract array index from path
     */
    static _getArrayIndex(path) {
