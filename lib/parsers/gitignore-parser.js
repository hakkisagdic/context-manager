...
class GitIgnoreParser {
    ...
    convertToRegex(pattern) {
        ...
        let regexPattern = pattern
            ...
            .replace(/\/\*\*\//g, '/__DOUBLESTAR__/') // /**/ in middle
            .replace(/^\*\*\//g, '__DOUBLESTAR__/') // **/ at start
            .replace(/\/\*\*$/g, '/__DOUBLESTAR_END__') // /** at end
            .replace(/\*\*/g, '__DOUBLESTAR_ALONE__') // remaining **
            .replace(/\*/g, '[^/]*') // single * → match filename chars
            .replace(/\?/g, '[^/]') // ? → match single char
            .replace(/__DOUBLESTAR__\//g, '(([^/]+/)*)') // /**/ → zero or more path segments
            .replace(/__DOUBLESTAR__/g, '(([^/]+/)*)') // **/ at start → zero or more path segments
            .replace(/__DOUBLESTAR_END__/g, '(/.*)?') // /** at end → optional trailing path
            .replace(/__DOUBLESTAR_ALONE__/g, '.*'); // remaining ** → match anything
    ...
    isIgnored(filePath, relativePath) {
        ...
        if (this.hasIncludeFile) {
            ...
            // Find the matching pattern for tracing
            for (const pattern of this.contextPatterns) {
                if (pattern.regex.test(relativePath) && !pattern.isNegation) {
                    matchedRule = pattern.original;
                    break;
                }
            }
