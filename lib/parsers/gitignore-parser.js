...
class GitIgnoreParser {
    ...
    convertToRegex(pattern) {
        ...
        // Handle leading **/ specially - matches at any depth including root
        ...
        let regexPattern = pattern
            .replace(/\*\*/g, '\x00DOUBLESTAR\x00')  // Replace ** with placeholder first
            .replace(/[.+^${}()|[\]\\]/g, '\\$&')    // Escape special regex chars
            .replace(/\*/g, '[^/]*')                  // Replace single * with non-slash matcher
            .replace(/\?/g, '[^/]')                   // Replace ? with single char matcher
            .replace(/\x00DOUBLESTAR\x00/g, '.*');    // Replace placeholder with .*

        if (hasLeadingDoubleStar) {
            // **/ means match at any depth, including root
