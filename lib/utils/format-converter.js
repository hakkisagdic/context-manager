...
class FormatConverter {
    ...
    parseYAML(yamlString) {
        ...
        const isTopLevelArray = lines.length > 0 && lines.every(line =>
    ...
    /**
     * Partial conversion - convert only selected parts of data
     * @param {string} input - Input data
     * @param {string} fromFormat - Source format
     * @param {string} toFormat - Target format
     * @param {Object} options - Partial conversion options
     * @param {Array} options.fields - Field names to include (for objects)
     * @param {Array} options.exclude - Field names to exclude
     * @param {number} options.limit - Limit number of items (for arrays)
     * @param {number} options.offset - Skip first N items (for arrays)
     * @param {boolean} options.deep - Apply filtering to nested objects
     * @returns {Object} Conversion result with partial data
     */
    convertPartial(input, fromFormat, toFormat, options = {}) {
        const { fields, exclude, limit, offset = 0, deep = false } = options;

        // Parse input
        const data = this.parse(input, fromFormat);

        // Apply partial filtering
        const partialData = this.applyPartialFilter(data, { fields, exclude, limit, offset, deep });

        // Convert filtered data
        const output = this.encode(partialData, toFormat);
    ...
    /**
     * Apply partial filter to data
     */
    applyPartialFilter(data, options) {
        const { fields, exclude, limit, offset, deep } = options;
        ...
        if (Array.isArray(data)) {
            ...
            if ((fields || exclude) && deep) {
                result = result.map(item =>
                    this.applyPartialFilter(item, { fields, exclude, deep })
                );
            }
        ...
        if (typeof data === 'object' && data !== null) {
            ...
            for (const [key, value] of Object.entries(data)) {
                ...
                // Apply deep filtering to nested objects
                if (deep && (typeof value === 'object' && value !== null)) {
                    result[key] = this.applyPartialFilter(value, { fields, exclude, deep });
                } else {
                    result[key] = value;
                }
            }
    ...
    /**
     * Convert with field projection (select specific fields)
     * @param {string} input - Input data
     * @param {string} fromFormat - Source format
     * @param {string} toFormat - Target format
     * @param {Array|Object} projection - Fields to include (array) or projection spec (object)
     */
    convertWithProjection(input, fromFormat, toFormat, projection) {
        const data = this.parse(input, fromFormat);
        const projected = this.projectFields(data, projection);
        const output = this.encode(projected, toFormat);
        ...
        return {
            ...
            metadata: {
                fromFormat,
                toFormat,
                projection: Array.isArray(projection) ? projection : Object.keys(projection)
            }
        };
    }
    ...
    projectFields(data, projection) {
        if (Array.isArray(projection)) {
            // Simple field list
            return this.applyPartialFilter(data, { fields: projection, deep: false });
        }

        // MongoDB-style projection: { field: 1, nested.field: 1 }
        const result = {};
        ...
        for (const [path, include] of Object.entries(projection)) {
            ...
            if (parts.length === 1) {
                ...
                if (current && parts[parts.length - 1] in current) {
                    ...
                    target[parts[parts.length - 1]] = current[parts[parts.length - 1]];
                }
            }
        }
