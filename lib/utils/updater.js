...
class Updater {
    constructor(options = {}) {
        ...
        // Update endpoints (can be overridden via registryUrl)
        const baseUrl = this.registryUrl || 'https://api.github.com/repos/hakkisagdic/context-manager';
        this.endpoints = {
            stable: `${baseUrl}/releases/latest`,
            insider: `${baseUrl}/releases`, // Get all releases
    ...
    /**
     * Fetch JSON from URL
     * Supports proxy and retry logic
     */
    async fetchJSON(url, retries = 3) {
        ...
        if (this.proxy) {
            // Note: Full proxy support requires http(s)-proxy-agent package
            // For minimal dependency setup, we store proxy for future use
            // Enterprise users can extend this with proper proxy agent
            console.log(`⚠️  Proxy configured but requires https-proxy-agent package: ${this.proxy}`);
        }
     ...
     * Supports semver pre-release versions (e.g., 3.1.0-beta.1)
     */
    compareVersions(v1, v2) {
        ...
        // No pre-release is greater than pre-release (stable > beta)
        if (!prerelease1 && prerelease2) return 1;  // v1 is stable, v2 is pre-release
        if (prerelease1 && !prerelease2) return -1; // v1 is pre-release, v2 is stable
        if (!prerelease1 && !prerelease2) return 0; // Both stable
