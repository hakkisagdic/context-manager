...
class MethodAnalyzer {
    ...
    extractMethods(content, filePath) {
        // Handle both file paths and direct extensions (for tests)
        let ext = path.extname(filePath).toLowerCase();
        if (!ext && filePath) {
            // If no extension found, assume filePath is the extension itself (e.g., 'ts', 'py')
        ...
        if (ext === '.rs') {
            ...
            return this.extractHTMLElements(content, filePath);
        } else if (ext === '.md' || ext === '.markdown') {
            return this.extractMarkdownSections(content, filePath);
        } else if (ext === '.xml') {
            return this.extractXMLElements(content, filePath);
        } else {
    ...
    extractJavaScriptMethods(content, filePath) {
        ...
        const patterns = [
            ...
            { regex: new RegExp(`(${namePattern})\\s*(?:<[^>]+>)?\\s*:\\s*(?:async\\s+)?function\\s*\\(`, 'g'), type: 'method' },
            { regex: new RegExp(`(?:const|let|var)\\s+(${namePattern})\\s*=\\s*(?:async\\s+)?(?:<[^>]+>)?\\s*\\([^)]*\\)\\s*=>`, 'g'), type: 'arrow' },
        ...
        const patterns = [
            ...
            // Matches: public/private/protected/internal static/async returnType MethodName(params) {
            {
                regex: new RegExp(
                    ...
                    `\\([^)]*\\)\\s*` +
                    `(?:where\\s+[^{=>]+)?\\s*` + // Generic constraints (allow => too)
                    `[{]`,
                    'g'
                ),
                type: 'method'
            },
            // Expression-bodied members: returnType MethodName(params) => expression
            {
                regex: new RegExp(
                    `(?:public|private|protected|internal)?\\s*` +
                    `(?:static|virtual|override|abstract|async)?\\s*` +
    ...
    extractPythonMethods(content, filePath) {
        ...
        const patterns = [
            // def function_name or async def function_name (with optional type hints)
            { regex: new RegExp(`^\\s*(?:async\\s+)?def\\s+(${namePattern})\\s*\\([^)]*\\)(?:\\s*->\\s*[^:]+)?\\s*:`, 'gm'), type: 'function' },
            // @decorator (any decorator including @staticmethod, @classmethod, @app.route, etc.)
            { regex: new RegExp(`@[\\w.]+(?:\\([^)]*\\))?\\s+(?:async\\s+)?def\\s+(${namePattern})\\s*\\(`, 'gm'), type: 'method' },
        ];
    ...
    extractKotlinMethods(content, filePath) {
        ...
        const patterns = [
            // fun functionName or fun ClassName.functionName (with suspend, inline, reified support)
    ...
    extractSwiftMethods(content, filePath) {
        ...
        const patterns = [
            // func functionName (with mutating, throws, async support)
            { regex: new RegExp(`(?:public|private|internal|fileprivate)?\\s*(?:static|class|mutating)?\\s*func\\s+(${namePattern})\\s*(?:<[^>]+>)?\\s*\\([^)]*\\)\\s*(?:throws|async)?`, 'g'), type: 'function' },
            // init (constructors)
    ...
    /**
     * Extract HTML elements (headings, sections, components, forms, scripts)
     * @param {string} content - HTML file content
     * @param {string} filePath - File path
     * @returns {Array} - Array of extracted elements
     */
    extractHTMLElements(content, filePath) {
        const elements = [];
        const lines = content.split('\n');

        // Pattern definitions for different HTML elements
        const patterns = [
            // Headings: <h1>, <h2>, <h3>, <h4>, <h5>, <h6>
            { regex: /<h([1-6])(?:\s+[^>]*)?>(.*?)<\/h\1>/gi, type: 'heading' },

            // Semantic sections: <section>, <article>, <aside>, <nav>, <header>, <footer>, <main>
            { regex: /<(section|article|aside|nav|header|footer|main)(?:\s+[^>]*)?(?:id|class)=["']([^"']+)["'][^>]*>/gi, type: 'section' },

            // Divs with id or class (components)
            { regex: /<div(?:\s+[^>]*)?(?:id|class)=["']([^"']+)["'][^>]*>/gi, type: 'component' },

            // Forms
            { regex: /<form(?:\s+[^>]*)?(?:id|name)=["']([^"']+)["'][^>]*>/gi, type: 'form' },

            // Scripts
            { regex: /<script(?:\s+[^>]*)?(?:id|src)=["']([^"']+)["'][^>]*>/gi, type: 'script' },

            // Custom elements (web components)
            { regex: /<([a-z]+-[a-z-]+)(?:\s+[^>]*)?>/gi, type: 'custom-element' },

            // Templates
            { regex: /<template(?:\s+[^>]*)?id=["']([^"']+)["'][^>]*>/gi, type: 'template' }
        ];
        ...
        patterns.forEach(({ regex, type }) => {
            ...
            while ((match = regex.exec(content)) !== null) {
                ...
                let name;
                if (type === 'heading') {
                    // For headings, extract the text content
                    const level = match[1];
                    const text = match[2].replace(/<[^>]+>/g, '').trim(); // Strip inner tags
                    name = text.substring(0, 50); // Limit to 50 chars
                    elements.push({
                        name: `h${level}: ${name}`,
                        line: lineNumber,
                        type: `h${level}`
                    ...
                    // For sections, use the tag name and id/class
                    const tagName = match[1];
                    const identifier = match[2];
                    elements.push({
                        name: `${tagName}#${identifier}`,
                        line: lineNumber,
                        type: tagName
                    });
                } else if (type === 'component') {
                    // For divs, use the class/id
                    ...
                    // For custom elements, use the tag name
                    const tagName = match[1];
                    elements.push({
                        name: tagName,
                        line: lineNumber,
                    ...
                    // For forms, scripts, templates - use id/name/src
                    const identifier = match[1];
                    elements.push({
                        name: identifier,
                        line: lineNumber,
    ...
    /**
     * Extract Markdown sections (headings, code blocks, tables, lists)
     * @param {string} content - Markdown file content
     * @param {string} filePath - File path
     * @returns {Array} - Array of extracted sections
     */
    extractMarkdownSections(content, filePath) {
        const sections = [];
        const lines = content.split('\n');
        ...
        lines.forEach((line, index) => {
            ...
            // Code blocks: ```language (only opening fence with language)
            else if ((match = /^```([a-zA-Z][\w+\-]*)/.exec(line))) {
                // Only capture opening fence with language specified
                // Ignore closing fences (``` alone) by requiring at least one letter after backticks
            ...
            else if (/^\|(.+)\|$/.test(line) && lineNumber === 1 ||
                     (lineNumber > 1 && /^\|[-:\s|]+\|$/.test(lines[lineNumber - 2]))) {
                // This is a table header
                const headers = line.split('|').filter(h => h.trim()).map(h => h.trim()).join(', ');
                sections.push({
                    name: `table: ${headers.substring(0, 40)}`,
                    line: lineNumber,
                    type: 'table'
                });
            }
            ...
            // Ordered lists: 1. Item
                ...
                const text = match[2].substring(0, 40);
                sections.push({
                    name: `list-item: ${text}`,
                    line: lineNumber,
                    type: 'ordered-list'
            ...
            // Unordered lists: - Item or * Item
                ...
                const text = match[1].substring(0, 40);
                sections.push({
                    name: `list-item: ${text}`,
                    line: lineNumber,
                    type: 'unordered-list'
    ...
    /**
     * Extract XML elements (root, major elements, elements with attributes)
     * @param {string} content - XML file content
     * @param {string} filePath - File path
     * @returns {Array} - Array of extracted elements
     */
    extractXMLElements(content, filePath) {
        const elements = [];

        // Pattern definitions for XML elements
        const patterns = [
            // Root element with namespace: <root xmlns="...">
            { regex: /<([a-zA-Z][\w:-]*)\s+xmlns[^>]*>/g, type: 'root-element' },

            // Elements with id attribute: <element id="...">
            { regex: /<([a-zA-Z][\w:-]*)\s+[^>]*id=["']([^"']+)["'][^>]*>/g, type: 'element-with-id' },

            // Elements with name attribute: <element name="...">
            { regex: /<([a-zA-Z][\w:-]*)\s+[^>]*name=["']([^"']+)["'][^>]*>/g, type: 'element-with-name' },

            // Processing instructions: <?xml-stylesheet ?>
            { regex: /<\?([a-zA-Z][\w-]*)\s+[^>]*\?>/g, type: 'processing-instruction' },

            // Comments with TODO/FIXME
            { regex: /<!--\s*(TODO|FIXME|NOTE|HACK|XXX):\s*([^-]+)-->/gi, type: 'comment' },

            // Namespaced elements: <ns:element>
            { regex: /<([a-zA-Z][\w-]*):([a-zA-Z][\w-]*)(?:\s+[^>]*)?>/g, type: 'namespaced-element' }
        ];
        ...
        patterns.forEach(({ regex, type }) => {
            ...
            while ((match = regex.exec(content)) !== null) {
                ...
                if (type === 'root-element') {
                    name = match[1];
                } else if (type === 'element-with-id') {
                    name = `${match[1]}#${match[2]}`;
                } else if (type === 'element-with-name') {
                    name = `${match[1]}[@name="${match[2]}"]`;
                } else if (type === 'processing-instruction') {
                    name = match[1];
                } else if (type === 'comment') {
                    const tag = match[1];
                    const text = match[2].trim().substring(0, 40);
                    name = `${tag}: ${text}`;
                } else if (type === 'namespaced-element') {
