...
class MethodAnalyzer {
    ...
    extractJavaScriptMethods(content, filePath) {
        ...
        const patterns = [
            ...
            // Support TypeScript generics: function name<T>(...) or function name(...)
    ...
    extractJavaMethods(content, filePath) {
        ...
        const patterns = [
            // Methods with return type (public/private/protected/static/final/etc.)
            // Match modifiers, optional generics, return type, method name
            {
                regex: new RegExp(`(?:(?:public|private|protected|static|final|synchronized|native|abstract)\\s+)+(?:<[^>]+>\\s+)?(?:[\\w<>\\[\\],]+)\\s+(${namePattern})\\s*\\(`, 'g'),
                type: 'method'
            },
            // Constructor (must have opening brace, modifiers optional)
            // Only matches if followed by {, to distinguish from method calls
            {
    ...
    extractPHPMethods(content, filePath) {
        ...
        const patterns = [
            // public/private/protected/static function method_name or plain function
            // Single pattern to avoid duplicates
            { regex: new RegExp(`(?:(?:public|private|protected)\\s+)?(?:static\\s+)?function\\s+(${namePattern})\\s*\\(`, 'g'), type: 'function' },
        ];
    ...
    extractKotlinMethods(content, filePath) {
        ...
        const patterns = [
            ...
            // Support generics: fun <T> ClassName<T>.functionName() or fun functionName()
            { regex: new RegExp(`(?:public|private|protected|internal)?\\s*(?:suspend\\s+)?(?:inline\\s+)?fun\\s+(?:<[^>]+>\\s+)?(?:[\\w<>.,\\s]+\\.)?\\s*(${namePattern})\\s*(?:<[^>]+>)?\\s*\\(`, 'g'), type: 'function' },
        ];
