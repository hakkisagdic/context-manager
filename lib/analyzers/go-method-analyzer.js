...
class GoMethodAnalyzer {
    extractMethods(content, filePath) {
        const patterns = [
            ...
            // Interface method declarations: MethodName(params) returnType (inside interfaces)
            // Matches both multiline and inline interface definitions
            { regex: /(?:interface\s*\{[^}]*\s+|^\s+)([a-zA-Z_][a-zA-Z0-9_]*)\s*\([^)]*\)\s*(?:\([^)]*\)|[a-zA-Z0-9_*\[\]]+)?/gm, type: 'interface' },
        ];
