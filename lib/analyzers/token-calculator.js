...
const RuleTracer = require('../debug/rule-tracer');
...
class TokenCalculator {
    ...
    run() {
        ...
        // Trace rules if requested
        if (this.options.traceRules) {
            this.traceRules(allFiles);
        }
    ...
    traceRules(allFiles) {
        const tracer = new RuleTracer(this.gitIgnore, this.methodFilter);

        console.log('\n🔍 Rule Tracing Mode\n');
        console.log('='.repeat(70));

        // Trace sample files (first 10)
        const sampleFiles = allFiles.slice(0, Math.min(10, allFiles.length));

        console.log('\n📁 Sample File Traces:\n');
        for (const file of sampleFiles) {
            const trace = tracer.traceFile(file, this.projectRoot);
            tracer.printTrace(trace);
        }

        // Analyze patterns
        const analysis = tracer.analyzePatterns(allFiles);
        tracer.printPatternAnalysis(analysis);

        console.log('💡 Tip: Run without --trace-rules to see full analysis\n');
    }
