...
class TokenCalculator {
    ...
    initMethodFilter() {
        return ConfigUtils.initMethodFilter(this.projectRoot, this.options.ruleTracer);
    }
    ...
    initGitIgnore() {
        return ConfigUtils.initGitIgnore(this.projectRoot, this.options.ruleTracer);
    }
    ...
    run() {
        ...
        // Apply token budget fitting if specified (before exports)
        let filesToExport = analysisResults;
        if (this.options.targetTokens && !this.options.dashboard) {
            filesToExport = this.applyTokenBudgetFitting(analysisResults);
        }

        // Handle exports (skip for dashboard mode)
        if (!this.options.dashboard) {
            this.handleExports(filesToExport);
        }

        // Return stats and files for programmatic usage
        // Spread stats for backward compatibility, add files array for token budget fitting
        return {
            ...this.stats,
            files: filesToExport
        };
    ...
    /**
     * Apply token budget fitting to analysis results
     * @param {Array} analysisResults - Original analysis results
     * @returns {Array} Fitted file list
     */
    applyTokenBudgetFitting(analysisResults) {
        try {
            console.log('\n🎯 Token Budget Fitting');
            console.log('═'.repeat(60));

            const fitter = new TokenBudgetFitter(this.options.targetTokens, this.options.fitStrategy);
            const fitResult = fitter.fitToWindow(analysisResults, {
            ...
            this.stats.totalTokens = fitResult.totalTokens;
            this.stats.totalFiles = fitResult.files.length;

            return fitResult.files;
        } catch (error) {
            console.error(`❌ Token budget fitting failed: ${error.message}`);
            console.error('   Continuing with original file list...\n');
            return analysisResults; // Return original files on error
        }
    }
